/* Customer-facing QR menu. Served at /menu/<token>. No login.
   AWS build: calls the API Gateway origin directly (no same-origin Hosting
   rewrite exists on CloudFront — see aws/infra/lib/hosting-stack.ts) — CORS
   is enabled on the HTTP API for exactly this reason (api-stack.ts). */
