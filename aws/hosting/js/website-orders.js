...
(async function boot() {
  ...
  async function refresh() {
    try {
      const { orders } = await apiFetch("/website-orders");
      ORDERS = orders.map(rowOf);
      render();
    } catch (e) { showToast(e.message, true); }
  }
  connectRealtime("website_orders", (msg) => {
    if (msg.type === "order.status" || msg.type === "order.items" || msg.type === "order.settled") refresh();
  }, { onReconnect: refresh });
  startReconciliationPoll(refresh, 30000);
})();

// `d` here is already the REST row shape from queries.listWebsiteOrders
// (snake_case, see aws/backend/src/handlers/callable/queries.ts websiteOrderRow) —
// this just flattens fulfillment/customer the way this page's render() expects.
function rowOf(d) {
  ...
  return {
    ...
    pickup_at: iso(d.fulfillment?.pickup_at),
    ...
    subtotal_paise: Number(d.subtotal_paise) || 0,
    tax_paise: Number(d.tax_paise) || 0,
    total_paise: Number(d.total_paise) || 0,
    advance_paise: Number(d.advance_paise) || 0,
    balance_paise: Number(d.balance_paise) || 0,
    paid_paise: Number(d.paid_paise) || 0,
    settled_bill_nos: d.settled_bill_nos || [],
    bill_status: (d.settled_bill_nos || []).length ? "billed" : "unbilled",
    kitchen_ticket_id: d.kitchen_ticket_id || null,
    kitchen_status: d.kitchen_status || null,
