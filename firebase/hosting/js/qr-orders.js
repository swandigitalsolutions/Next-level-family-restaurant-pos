...
function render(orders) {
  const signature = orders.map((o) => `${o.id}:${o.status}:${o.kitchen_status || ""}`).join("|");
    ...
    return `
      <div class="order-card${fresh}" data-status="${o.status}">
        ...
        <div class="oc-actions">
          ...
          ${!o.kitchen_ticket_id && canCancel ? `<button class="btn btn-outline" data-accept="${o.id}">Accept → Kitchen</button>` : ""}
  ...
  board.querySelectorAll("[data-accept]").forEach((b) =>
    b.addEventListener("click", () => acceptToKitchen(b.dataset.accept)));
}

async function acceptToKitchen(id) {
  try {
    await apiFetch(`/qr-ordering/orders/${id}/accept`, { method: "POST" });
    showToast("Sent to the kitchen screen");
    lastSignature = "";
    await load(true);
  } catch (e) {
