/* Kitchen screen — the separate UI for the `kitchen` role (also reachable by
   manager/admin). Realtime board of tickets that billing has "accepted" from
   the QR-table and Website order flows. New tickets ring a chime. No prices,
   no bill — just what to cook and how far along it is. */
...
(async function boot() {
  ...
  onSnapshot(
    ...
    (snap) => {
      TICKETS = snap.docs.map((d) => rowOf(d.id, d.data()));
      // metadata.fromCache is true on the first cached delivery and after a
      // reconnect replay — don't chime for those, just seed state.
