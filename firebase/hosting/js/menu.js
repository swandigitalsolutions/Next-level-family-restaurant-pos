(async function initMenu() {
  ...
  async function loadAll() {
    try {
      const [foodCats, foodItems, alcCats, alcItems, cafeCats, cafeItems] = await Promise.all([
  ...
  function renderFoodCats() {
    ...
    const categoryCount = FOOD_CATS.filter(c => c.status === "active").length + ALCOHOL_CATS.filter(c => c.status === "active").length + CAFE_CATS.filter(c => c.status === "active").length;
  ...
  const catsFor = (type) => (type === "food" ? FOOD_CATS : type === "cafe" ? CAFE_CATS : ALCOHOL_CATS);
  const renderCatsFor = (type) => (type === "food" ? renderFoodCats() : type === "cafe" ? renderCafeCats() : renderAlcoholCats());
  ...
  function startEditCategory(row, type) {
    ...
    actionsCell.querySelector(".act-cancel-cat").addEventListener("click", () => renderCatsFor(type));
  }
  ...
  // ================================================================
  // CAFE (outside-cafe menu: tea/coffee/ice cream/bottled + cool drinks)
  // Same shape as food — no brand/bottle/tax.
  // ================================================================

  function renderCafeCatSelect() {
    const sel = document.getElementById("cafeItemCategorySelect");
    sel.innerHTML = CAFE_CATS.filter(c => c.status === "active").map(c => `<option value="${c.id}">${escapeHtml(c.name)}</option>`).join("");
  }

  function renderCafeCats() {
    const body = document.getElementById("cafeCatsBody");
    if (CAFE_CATS.length === 0) {
      body.innerHTML = `<tr><td colspan="3" class="empty-state">No cafe categories yet.</td></tr>`;
      return;
    }
    body.innerHTML = CAFE_CATS.map(c => `
      <tr data-id="${c.id}">
        <td class="cell-name">${escapeHtml(c.name)}</td>
        <td><span class="tag ${c.status === 'active' ? 'tag-active' : 'tag-inactive'}">${c.status}</span></td>
        <td class="row-actions">
          <button class="btn btn-outline btn-sm act-edit-cat" data-type="cafe">Edit</button>
          <button class="btn btn-danger btn-sm act-delete-cat" data-type="cafe">Delete</button>
        </td>
      </tr>
    `).join("");
    wireCategoryRowActions(body, "cafe");
  }

  document.getElementById("cafeCatForm").addEventListener("submit", async (e) => {
    e.preventDefault();
    const name = e.target.name.value.trim();
    if (!name) return;
    try {
      await apiFetch("/cafe/categories", { method: "POST", body: { name } });
      showToast("Cafe category added");
      e.target.reset();
      await loadAll();
  ...
  function renderCafeItems() {
    const body = document.getElementById("cafeItemsBody");
    const query = (document.getElementById("cafeCatalogSearch")?.value || "").toLowerCase().trim();
    const filtered = CAFE_ITEMS.filter(i => !query || `${i.name} ${i.category_name}`.toLowerCase().includes(query));
    document.getElementById("cafeTabCount").textContent = CAFE_ITEMS.length;
    document.getElementById("menuCafeCount").textContent = CAFE_ITEMS.filter(i => i.status === "active").length;
    if (filtered.length === 0) {
      body.innerHTML = `<div class="empty-state card-empty">${query ? "No cafe items match that search." : "No cafe items yet."}</div>`;
      return;
    }
    body.innerHTML = filtered.map(i => `
      <article class="menu-admin-card" data-id="${i.id}">
    ...
    `).join("");
    body.querySelectorAll(".act-edit-item").forEach(btn => btn.addEventListener("click", () => startEditCafeItem(btn.closest(".menu-admin-card"))));
    body.querySelectorAll(".act-delete-item").forEach(btn => btn.addEventListener("click", () => deleteCafeItem(btn.closest(".menu-admin-card").dataset.id)));
  }

  function startEditCafeItem(row) {
    const id = row.dataset.id;
    const item = CAFE_ITEMS.find(i => String(i.id) === String(id));
    if (!item) return;
    const catOptions = CAFE_CATS.filter(c => c.status === "active")
      .map(c => `<option value="${c.id}" ${c.id === item.category_id ? "selected" : ""}>${escapeHtml(c.name)}</option>`).join("");
    row.querySelector(".cell-name").innerHTML = `<input class="edit-input edit-name" value="${escapeHtml(item.name)}" />`;
    row.querySelector(".cell-category").innerHTML = `<select class="edit-input edit-cat">${catOptions}</select>`;
    row.querySelector(".cell-price").innerHTML = `<input class="edit-input edit-price" type="number" step="0.01" min="0" value="${item.price}" />`;
    row.querySelector(".cell-stock").innerHTML = `<input class="edit-input edit-stock" type="number" step="1" min="0" placeholder="Stock (blank = unlimited)" value="${item.stock_qty ?? ""}" />`;
    ...
    actionsCell.querySelector(".act-save-item").addEventListener("click", async () => {
      const name = row.querySelector(".edit-name").value.trim();
      const category_id = row.querySelector(".edit-cat").value;
      const price = row.querySelector(".edit-price").value;
      const stock_qty = row.querySelector(".edit-stock").value;
      if (!name) { showToast("Item name is required", true); return; }
      try {
        await apiFetch(`/cafe/items/${id}`, { method: "PUT", body: { name, category_id, price, stock_qty } });
        showToast("Item updated");
        await loadAll();
      } catch (err) { showToast(err.message, true); }
    });
    actionsCell.querySelector(".act-cancel-item").addEventListener("click", renderCafeItems);
  }

  async function deleteCafeItem(id) {
    const ok = await confirmAction({
      title: "Remove this item?",
      body: "It will no longer appear on the cafe billing screen.",
      confirmLabel: "Remove item", danger: true,
    });
    if (!ok) return;
    try {
      await apiFetch(`/cafe/items/${id}`, { method: "DELETE" });
      showToast("Item removed");
      await loadAll();
    } catch (err) { showToast(err.message, true); }
  }

  document.getElementById("cafeItemForm").addEventListener("submit", async (e) => {
    e.preventDefault();
    const form = e.target;
    const payload = { name: form.name.value.trim(), category_id: form.category_id.value, price: form.price.value, stock_qty: form.stock_qty.value };
    if (!payload.name || !payload.category_id) { showToast("Name and category are required", true); return; }
    try {
      await apiFetch("/cafe/items", { method: "POST", body: payload });
  ...
  document.getElementById("cafeCatalogSearch")?.addEventListener("input", renderCafeItems);
  ...
  document.getElementById("focusCafeForm")?.addEventListener("click", () => { document.getElementById("cafeAddPanel").scrollIntoView({ behavior: "smooth", block: "center" }); document.querySelector('#cafeItemForm input[name="name"]').focus(); });
